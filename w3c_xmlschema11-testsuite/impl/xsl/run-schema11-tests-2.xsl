...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"                
		  ...
		  <body>	  
			 ...
			 <table border="1">
				...
				<xsl:variable name="totalTests">
                   <xsl:value-of select="count(document(ts:testSetRef/@xlink:href)//ts:schemaTest[(@version = '1.1') or (ts:expected/@version = '1.1')] | 
				                               document(ts:testSetRef/@xlink:href)//ts:instanceTest[(@version = '1.1') or (ts:expected/@version = '1.1')])" />											   
				</xsl:variable>
	...
	<xsl:template match="@xlink:href" mode="testSet">
	   ...
	   <xsl:if test="$testSetDoc//@version = '1.1'">
	      <tr>
	         <td>&#160;</td>
	      ...
	      <xsl:apply-templates select="$testSetDoc/ts:testSet/ts:testGroup[.//@version = '1.1']" />
	   </xsl:if>
	</xsl:template>
	...
	<xsl:template match="ts:testGroup">
	      ...
	      <xsl:variable name="schemaTestResult">
	         <xsl:apply-templates select="ts:schemaTest[(@version = '1.1') or (ts:expected/@version = '1.1')]"/>
	      </xsl:variable>
	      ...
	      <xsl:apply-templates select="ts:instanceTest[(@version = '1.1') or (ts:expected/@version = '1.1')]">
	...
	<xsl:template match="ts:schemaTest">   
	   <tr>
		 ...
		 <td align="center"><xsl:value-of select="count(preceding::ts:schemaTest[(@version = '1.1') or (ts:expected/@version = '1.1')] | preceding::ts:instanceTest[(@version = '1.1') or (ts:expected/@version = '1.1')]) + 1" />.</td>
		 <td>&#160;</td>
		 <td>&#160;</td>
		 <td align="center"><xsl:value-of select="@name" /></td>
		 <td>&#160;</td>
		       ...
		       <xsl:variable name="instanceDocValidity">
                  <xsl:choose>
			         <xsl:when test="document(ts:instanceDocument/@xlink:href)/*/@xsi:schemaLocation or 
				          ...
				          <xsl:value-of select="java:XMLValidatorHelper2.getInstanceDocValidity(ts:instanceDocument/@xlink:href, 'null')" />
				     </xsl:when>
				     <xsl:otherwise>
				         <xsl:value-of select="java:XMLValidatorHelper2.getInstanceDocValidity(ts:instanceDocument/@xlink:href,
