...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"                
	...
	<xsl:variable name="undecidedTests">
	   <!-- write 'valid' values in attributes "id" and "name" to enable this feature. 
	        exclude="true" doesn't show the results in the report.
	   -->
	   <vendor id="SAXONICA">
	      <testSet name="VC">
	        <testGroup name="vc902" exclude="true"/>
		  </testSet>
	   </vendor>
	</xsl:variable>
	...
	<xsl:template match="ts:testGroup">
	   ...
	   <!--<xsl:variable name="isExcludeFromRunning" select="@name = exslt:node-set($undecidedTests)/vendor[@id = $vendorId]/testSet[@name = $testSetContextNode/@name]/testGroup[@exclude = 'true']/@name"/>-->
	      <!--<xsl:variable name="testGroupClrCode">
	         <xsl:choose>
		         <xsl:when test="@name = exslt:node-set($undecidedTests)/vendor[@id = $vendorId]/testSet[@name = $testSetContextNode/@name]/testGroup/@name">
			        <xsl:value-of select="exslt:node-set($testColorCode)/undecided"/>
			     </xsl:when>
			     <xsl:otherwise/>
		     </xsl:choose>
	      </xsl:variable> -->
	...
	<xsl:template match="ts:schemaTest">   
	   <tr>
		 ...
		 <td align="center"><xsl:value-of select="@name" /></td>
		 <td>&#160;</td>
		 <xsl:variable name="isXS11Version">
		    <xsl:call-template name="getSchemaLanguageVersion">
			   <xsl:with-param name="testVersion" select="@version"/>
			   <xsl:with-param name="testGroupVersion" select="../@version"/>
			   <xsl:with-param name="testSetVersion" select="/ts:testSet/@version"/>
			</xsl:call-template>
		 </xsl:variable>
		 <xsl:variable name="schemaDocValidity">
		    <xsl:choose>
			   <xsl:when test="java:XMLValidatorHelper1.isSchemaValid(ts:schemaDocument[1]/@xlink:href, $isXS11Version, /ts:testSet/@name)">
			      valid
			   </xsl:when>
			   <xsl:otherwise>
			      invalid
		 ...
		 <!-- determine effective value of "expectedValidity" -->
		 <xsl:variable name="effectiveExpValidity">
		    <xsl:choose>
		       <xsl:when test="($vendorId = 'SAXONICA') and (/ts:testSet/@name = 'CTA')">
			      <xsl:choose>
			         <xsl:when test="ts:expected[@version = 'full-xpath-in-CTA']">
						<xsl:value-of select="ts:expected[@version = 'full-xpath-in-CTA']/@validity"/>
				     </xsl:when>
				     <xsl:otherwise>
					    <xsl:variable name="xs11ExpectedValidity">
						   <xsl:call-template name="getXS11ExpectedValidity">
						      <xsl:with-param name="expectedValidity" select="ts:expected"/>
						   </xsl:call-template>
						</xsl:variable>
				        <xsl:value-of select="normalize-space($xs11ExpectedValidity)"/>
				     </xsl:otherwise>
				  </xsl:choose>
			   </xsl:when>
			   <xsl:otherwise>
	   ...
	   <tr>
		 ...
		 <xsl:choose>
		    <xsl:when test="$schDocValidity = 'valid'">
		       <xsl:variable name="instanceDocValidity">
                  <xsl:choose>
			         <xsl:when test="document(ts:instanceDocument/@xlink:href)/*/@xsi:schemaLocation or 
				          ...
				          <xsl:value-of select="java:XMLValidatorHelper1.getInstanceDocValidity(ts:instanceDocument/@xlink:href, 'null' , $isXS11Version, $vendorId, /ts:testSet/@name)" />
				     </xsl:when>
				     <xsl:otherwise>
				         <xsl:value-of select="java:XMLValidatorHelper1.getInstanceDocValidity(ts:instanceDocument/@xlink:href,
			                                   preceding-sibling::ts:schemaTest[1]/ts:schemaDocument[1]/@xlink:href, $isXS11Version, $vendorId, /ts:testSet/@name)" />
				     </xsl:otherwise>
			      </xsl:choose>
		       </xsl:variable>
		       <xsl:variable name="clrStatus">
		          <xsl:call-template name="getTestCaseColorStatus">
			        <xsl:with-param name="expectedValidity" select="normalize-space($xs11ExpectedValidity)" />
			        <xsl:with-param name="actualValidity" select="normalize-space($instanceDocValidity)" />
			      </xsl:call-template>
		       </xsl:variable>
		       <td align="center" bgcolor="{normalize-space($clrStatus)}"><xsl:value-of select="normalize-space($xs11ExpectedValidity)" /></td>
		       ...
		       <xsl:if test="normalize-space($instanceDocValidity) = normalize-space($xs11ExpectedValidity)">
	              <td id="pass" />
	           </xsl:if>
		    </xsl:when>
		    <xsl:otherwise>
			    <!-- if schema is 'invalid', don't run the corresponding instance tests and set instance test results as 'pass' -->
		        <td align="center" bgcolor="{exslt:node-set($testColorCode)/pass}"><xsl:value-of select="normalize-space($xs11ExpectedValidity)" /></td>
		        <td align="center" bgcolor="{exslt:node-set($testColorCode)/pass}"><xsl:value-of select="normalize-space($xs11ExpectedValidity)" /></td>
				<!-- a hidden table cell to track test result, status -->
		        <td id="pass" />
		    </xsl:otherwise>
	...
	<!--
	   Get effective value of expected validity.
	-->
	<xsl:template name="getXS11ExpectedValidity">
	   <xsl:param name="expectedValidity"/>
	   
	   <xsl:variable name="preferredValidityChoice" select="$expectedValidity[@version = '1.1']/@validity"/>
	   <xsl:choose>
		  <xsl:when test="$preferredValidityChoice">
             <!-- a 1.1 validity value is present -->			   
			 <xsl:value-of select="$preferredValidityChoice"/>
		  </xsl:when>
		  <xsl:otherwise>
             <!-- assuming there is only one instanceTest/expected node present. -->		   
	...
	<!--
	   Check if XSD 1.1 processing is required. Doesn't read "version" attribute on element ts:testSuite yet.
	-->
	<xsl:template name="getSchemaLanguageVersion">
	   <xsl:param name="testVersion"/>
	   <xsl:param name="testGroupVersion"/>
	   <xsl:param name="testSetVersion"/>
	   
	   <xsl:choose>
	      <xsl:when test="$testVersion = '1.0'">
		     <xsl:value-of select="false()"/>
		  </xsl:when>
		  <xsl:when test="$testVersion = '1.1'">
		     <xsl:value-of select="true()"/>
		  </xsl:when>
		  <xsl:otherwise>
		      <xsl:choose>
	             <xsl:when test="$testGroupVersion = '1.0'">
		             <xsl:value-of select="false()"/>
		         </xsl:when>
				 <xsl:when test="$testGroupVersion = '1.1'">
		             <xsl:value-of select="true()"/>
		         </xsl:when>
		         <xsl:otherwise>
		            <xsl:choose>
	                   <xsl:when test="$testSetVersion = '1.0'">
		                  <xsl:value-of select="false()"/>
		               </xsl:when>
					   <xsl:when test="$testSetVersion = '1.1'">
		                  <xsl:value-of select="true()"/>
		               </xsl:when>
		               <xsl:otherwise>
		                  <xsl:value-of select="true()"/>
